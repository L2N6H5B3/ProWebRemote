...
function onMessage(evt) {
    ...
    if (obj.action == "authenticate" && obj.authenticated == "1") {
        ...
        if (location.charAt(0) == '0') {
            ...
            $(".libraries").children("div").children("a").children("div").addClass("selected");
                                    ...
                                    $(this.presentation.presentationSlideGroups).each (
                                        function () {
                                            $(this.groupSlides).each (
                                                function () {
                                                    data += '<div class="slide-container"><a onclick="triggerSlide(this);"><div id="'+location+'" class="slide"><img src="data:image/png;base64,'+this.slideImage+'" draggable="false"/><div class="slide-number">'+count+'</div></div></a></div>';
                                                    count ++;
                                                }
                                            );
                                        }
...
function triggerSlide(obj) {
    ...
    var location = ($(obj).children("div").attr("id")).replace(/\//g, "\\/");
    // var location = "0.0:0";
    index = $(obj).children("div").children("div").text() - 1;
